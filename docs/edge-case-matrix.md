...
| **Cloud `repeat-offender` tag** | same fingerprint processed twice on Cloud | n8n Code nodes are stateless — no in-process recurrence counter; tag is **API-only** (`POST /enrich`) | ⚙️ documented — dashboard may show repeat via fingerprint clustering from Sheet history |
