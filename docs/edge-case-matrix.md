...
| Oversized document beyond context | very large input | `prepare.js` truncates text at 900k chars with an inline warning; PDFs rely on Gemini inline upload limits | ⚙️ `MAX_DOCUMENT_CHARS` in prepare.js |
...
| **Cloud retry / partial failure** | Gemini succeeds but Sheets/Gmail fails; n8n retries the branch | may append a duplicate Sheet row or send a second email — no `document_id` lookup dedupe on Cloud | ⚙️ known limitation — see `docs/SETUP-GUIDE.md` |
| **Cloud zero-byte upload** | empty file on the grading form | `prepare.js` throws `Uploaded file is empty (0 bytes)` before Gemini | ✅ `test_prepare.mjs` |
| **Cloud unsupported extension** | `.exe`, `.docx`, etc. on the grading form | `prepare.js` rejects with a clear error (DOCX → self-hosted path documented) | ✅ `test_prepare.mjs` |
| **Cloud MIME/extension mismatch** | `.txt` file with `application/pdf` MIME | extension wins — routed as text, not PDF | ✅ `test_prepare.mjs` `prepare.mime_mismatch_text` |
| **Nested ZIP archives** | zip containing another zip | not supported — only top-level `.md`/`.txt` entries are extracted | ⚙️ documented in `docs/SETUP-GUIDE.md` |
| **Cloud `repeat-offender` tag** | same fingerprint processed twice on Cloud | n8n Code nodes are stateless — no in-process recurrence counter; tag is **API-only** (`POST /enrich`). `EnrichedResult.recurrence_seen_count` is therefore **optional (default 0)** so the Python brain ↔ `enrich.js` output schemas stay in lockstep (enrich.js omits it). | ⚙️ documented — dashboard may show repeat via fingerprint clustering from Sheet history |
